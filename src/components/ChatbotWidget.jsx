      ...
      <div className="space-y-3">
        {parts.map((part, pIdx) => {
          if (part.type === 'code') {
            ...
            return (
              <div key={blockId} className="relative my-2 rounded-xl bg-stone-900 border border-stone-800 overflow-hidden text-xs">
                <div className="flex items-center justify-between px-3 py-1.5 bg-stone-950/80 border-b border-stone-800 text-stone-400 font-mono-code text-[11px]">
                  <span>Code Snippet</span>
                  <button
                    onClick={() => handleCopyCode(part.content, blockId)}
                    className="flex items-center space-x-1 text-stone-300 hover:text-amber-300 transition-colors cursor-pointer"
                  >
                ...
                <pre className="p-3.5 text-amber-200 font-mono-code leading-relaxed overflow-x-auto whitespace-pre">
            ...
            <div key={pIdx} className="space-y-1.5">
              {lines.map((line, lIdx) => {
                ...
                if (line.trim().startsWith('>')) {
                  const content = line.trim().replace(/^>\s*/, '');
                  return (
                    <div key={lIdx} className="border-l-3 border-amber-600 bg-amber-50/60 p-2.5 rounded-r-xl italic text-stone-800 text-[12px] my-1">
    ...
    return boldParts.map((part, i) => {
      ...
      return codeParts.map((cPart, j) => {
        if (cPart.startsWith('`') && cPart.endsWith('`')) {
          return (
            <code key={j} className="bg-stone-100 text-amber-900 border border-stone-200 font-mono-code text-[11px] px-1.5 py-0.5 rounded-md">
              {cPart.slice(1, -1)}
            </code>
          );
        }
  ...
  return (
    <>
      ...
      {!isOpen && (
        <button
          onClick={onToggle}
          className="fixed bottom-6 right-6 z-50 flex items-center space-x-2.5 bg-stone-900 hover:bg-amber-700 text-stone-100 px-4 py-3 rounded-full shadow-2xl border border-stone-800 transition-all duration-300 transform hover:scale-105 cursor-pointer group"
          title="Ask AI Mentor a Doubt"
        >
          <div className="relative">
            <div className="w-8 h-8 rounded-full bg-amber-600/30 text-amber-300 flex items-center justify-center border border-amber-500/50">
              <MessageSquare className="w-4 h-4 text-amber-300 group-hover:rotate-12 transition-transform" />
            </div>
            <span className="absolute -top-1 -right-1 w-3 h-3 bg-emerald-500 rounded-full border-2 border-stone-900 animate-pulse"></span>
          </div>
          <div className="text-left pr-1">
            <span className="block text-xs font-bold leading-tight">Clear Doubts</span>
            <span className="block text-[10px] text-amber-300 font-medium">AI {activePersonaObj.name} {activePersonaObj.icon}</span>
          </div>
        </button>
      )}
      ...
      {isOpen && (
        <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-50 w-[94vw] sm:w-[440px] h-[580px] max-h-[85vh] bg-white rounded-3xl border border-stone-200 shadow-2xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-200 font-sans">
          ...
          <div className="bg-stone-900 text-stone-100 px-5 py-3.5 flex items-center justify-between border-b border-stone-800">
            <div className="flex items-center space-x-3">
              <div className="relative">
                ...
                <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 bg-emerald-400 rounded-full border-2 border-stone-900"></span>
              </div>
              <div>
                ...
                <p className="text-[11px] text-stone-400 truncate max-w-[210px]">
                  Topic: <strong className="text-stone-200">{currentTopic?.title || onboardingData.goal}</strong>
                </p>
              </div>
            </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-[#faf9f6]">
            ...
            {messages.map((msg) => (
              ...
              >
                ...
                <div className={`rounded-2xl px-4 py-3 text-xs ${
                  msg.sender === 'user'
                    ? 'bg-amber-700 text-white font-medium rounded-tr-none shadow-xs'
                    : 'bg-white text-stone-800 border border-stone-200 shadow-sm rounded-tl-none space-y-2'
                }`}>
                  {msg.sender === 'user' ? (
                  ...
                  <span className={`block text-[10px] mt-1 text-right ${
                    msg.sender === 'user' ? 'text-amber-200/80' : 'text-stone-400'
                  }`}>
                    {msg.timestamp}
                  </span>
                </div>
            ...
            {isTyping && (
              <div className="flex gap-2.5 max-w-[80%] mr-auto items-center">
                <div className="w-7 h-7 rounded-xl bg-stone-900 text-amber-300 flex items-center justify-center shrink-0 text-xs shadow-xs">
                  {activePersonaObj.icon}
                </div>
                <div className="bg-white border border-stone-200 rounded-2xl rounded-tl-none px-4 py-3 text-xs shadow-xs flex items-center space-x-2">
                  <span className="text-stone-500 font-medium">{activePersonaObj.name} is thinking</span>
          ...
          <div className="px-3 py-2 bg-white border-t border-stone-100 overflow-x-auto whitespace-nowrap scrollbar-none flex space-x-2">
            {suggestedDoubts.map((s, idx) => (
              <button
                key={idx}
                onClick={() => handleSendMessage(s.text)}
                disabled={isTyping}
                className="shrink-0 bg-stone-100 hover:bg-amber-100 text-stone-700 hover:text-amber-900 border border-stone-200 hover:border-amber-300 text-[11px] px-2.5 py-1 rounded-full font-medium transition-all cursor-pointer disabled:opacity-50"
              >
                {s.label}
              </button>
            ))}
          ...
          <form onSubmit={handleSubmit} className="p-3 bg-white border-t border-stone-200 flex items-center space-x-2">
            <input
              ...
              disabled={isTyping}
              className="flex-1 bg-stone-50 border border-stone-300 focus:border-amber-600 focus:bg-white text-stone-900 text-xs rounded-xl px-3.5 py-2.5 focus:outline-hidden transition-all placeholder:text-stone-400"
            />

            <button
              type="submit"
              disabled={!inputQuery.trim() || isTyping}
              className="bg-stone-900 hover:bg-amber-700 disabled:bg-stone-300 text-white p-2.5 rounded-xl transition-colors cursor-pointer shrink-0"
