...
export default function StudentDashboard({
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6 md:py-8 space-y-6 md:space-y-8 animate-in fade-in duration-300 font-sans">
      ...
      <div className="bg-[#DEB887] border border-[#C59B67] text-[#1A0F05] p-5 md:p-6 rounded-2xl space-y-4 shadow-md">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <div className="flex items-center space-x-2 text-[#5A2A00] text-xs font-bold uppercase tracking-wider mb-1">
              <Sparkles className="w-4 h-4 text-[#5A2A00]" />
              <span>AI Learning Companion Active</span>
            </div>
            <h1 className="text-2xl md:text-3xl font-bold text-[#1A0F05]">
              {greetingText}
            </h1>
            <p className="text-xs text-[#3D2714] mt-1">
              You're <strong className="text-[#5A2A00] font-bold">72% through</strong> your Python fundamentals path. You have a <strong className="text-[#5A2A00] font-bold">{studentProfile?.streak || 5}-day streak</strong>!
            </p>
          </div>

          <div className="flex items-center space-x-3 bg-[#F5E2C8] px-4 py-2.5 rounded-xl border border-[#C59B67] self-start sm:self-auto text-xs">
            <Flame className="w-5 h-5 text-[#8B3E00] fill-[#8B3E00]" />
            <div>
              <span className="text-[#5C4228] text-[10px] block font-medium">Streak</span>
              <span className="font-bold text-[#1A0F05]">{studentProfile?.streak || 5} Days Active</span>
            </div>
          </div>
        </div>

        {/* PHONE-FIRST QUICK ACTION BAR */}
        <div className="pt-3 border-t border-[#C59B67] grid grid-cols-2 sm:grid-cols-4 gap-2.5">
          <button
            onClick={() => onNavigate('ai-tutor')}
            className="flex items-center space-x-2.5 bg-[#FFF8F0] hover:bg-[#F5E2C8] p-3 rounded-xl border border-[#C59B67] transition-all cursor-pointer group shadow-2xs"
          >
            <div className="w-8 h-8 rounded-lg bg-[#F5E2C8] text-[#5A2A00] flex items-center justify-center shrink-0 border border-[#DDB480]">
              🎤
            </div>
            <div className="text-left">
              <span className="text-xs font-bold text-[#1A0F05] block group-hover:text-[#5A2A00]">Ask AI Voice</span>
              <span className="text-[10px] text-[#5C4228] block">Tap to speak</span>
            </div>
          </button>
          ...
          >
            ...
            <div className="text-left">
              <span className="text-xs font-bold text-[#1A0F05] block group-hover:text-[#5A2A00]">Scan Question</span>
              <span className="text-[10px] text-[#5C4228] block">Learn camera</span>
            </div>
          </button>
          ...
          >
            ...
            <div className="text-left">
              <span className="text-xs font-bold text-[#1A0F05] block group-hover:text-[#5A2A00]">Practice Quiz</span>
              <span className="text-[10px] text-[#5C4228] block">Adaptive Engine</span>
            </div>
          </button>
          ...
          >
            ...
            <div className="text-left">
              <span className="text-xs font-bold text-[#1A0F05] block group-hover:text-[#5A2A00]">Continue</span>
              <span className="text-[10px] text-[#5C4228] block">Python functions</span>
            </div>
          </button>
        </div>
      </div>
